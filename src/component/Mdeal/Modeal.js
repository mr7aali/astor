...
const Modeal = ({ modealData,refetch  }) => {
    ...
    const handleDelet=(email,price)=>{
        fetch(`https://astor-server-ibv9wp3q8-mr7aali.vercel.app/deleteAddvertise?email=${email}&price=${price}`, {
            method: 'DELETE'
        })
        .then(res=>res.json())
        .then(data=>{
    ...
    const hadleStutaus = (email, price) => {
        fetch(`https://astor-server-ibv9wp3q8-mr7aali.vercel.app/updateproductstatus?email=${email}&price=${price}`, {
            method: 'PUT'
        })
            .then(res => res.json())
    ...
    const handleModeal = data => {
        ...
        fetch('https://astor-server-ibv9wp3q8-mr7aali.vercel.app/booking', {
            method: 'POST',
            headers: {
